...
public class QNManageLog {
	...
	static {
		if (mPackageName.equals(DEFAULT_TAG)) {
			Context context = BrowserApplication.getAppContext();
			if (null != context) {
	...
	public static void initFileQueue() {
		...
		if (lens > 0) {
			...
			if (lens > FILENUM) {
				...
				for (int i = 0; i < delFileCount; i++) {// 删掉最新的FILENUM个日志文件之前的日志文件
					File deleteFile = new File(logfile[i].getAbsolutePath()
							.toString());
					deleteFile.delete();
				}
				for (int j = delFileCount; j < lens; j++) {// 将最新的FILENUM个日志文件的名字放入fileQueue
					try {
				...
				for (int j = 0; j < lens; j++) {// 若所有日志文件<FILENUM，则将所有日志文的名字均放入fileQueue
					try {
						fileQueue.put(logfile[j].getName().toString());
					} catch (InterruptedException e) {
						e.printStackTrace();
...
class ComparatorByDate_ implements Comparator<File> {
	...
	public int compare(File lhs, File rhs) {
		SimpleDateFormat sdf = new SimpleDateFormat(QNManageLog.DateFormat);
		try {
